...
static const ephemeris_t gps_eph = {
    ...
    .health_bits = 0,
    .kepler =
        { .tgd_gps_s = 5.122274160385132E-9,
          .crc = 198.9375,
          .crs = 10.28125,
...
START_TEST(test_ephemeris_almanac_divergence)
{
  ...
  for (u8 i = 0; i < 3;
    ...
    bool calc_alm_ok = (0 == calc_sat_state_almanac(
        &gps_alm, &t, alm_sat_pos, _, _, _, _));
    bool calc_eph_ok = (0 == calc_sat_state_n(
        &gps_eph, &t, eph_sat_pos, _, _, _, _, &iodc, &iode));
    bool calc_eph_div_ok = (0 == calc_sat_state_n(
    ...
    if (!calc_alm_ok || !calc_eph_ok || !calc_eph_div_ok) {
      fail_unless(false, "Failure computing sat state! \n"
          "Alm success: %" PRIu8" \n"
          "Eph success: %" PRIu8" \n"
          "Eph diverged success: %" PRIu8" \n",
          calc_alm_ok, calc_eph_ok, calc_eph_div_ok);
    }
    ...
    fail_unless(d <= VALID_ALM_ACCURACY,
                ...
                i,
                d,
                alm_sat_pos[0], div_sat_pos[0],
                alm_sat_pos[1], div_sat_pos[1],
                alm_sat_pos[2], div_sat_pos[2]);
    ...
    fail_unless(d <= VALID_EPH_ACCURACY,
                ...
                i,
                d,
                eph_sat_pos[0], div_sat_pos[0],
                eph_sat_pos[1], div_sat_pos[1],
                eph_sat_pos[2], div_sat_pos[2]);
  }
}
END_TEST
