...
#define fail_unless_mic(cond, format, ...) do{ if(!(cond)){fprintf(stderr, format, ##__VA_ARGS__);fprintf(stderr, "\n");} }while(0)
...
START_TEST(test_signal_aggregates)
{
  ...
  for (u32 k=0; k<ARRAY_COUNT(constellation_code_counts); k++) {
    fprintf(stderr, "  constellation_code_counts[%d] = %u\n", k, constellation_code_counts[k]);
  }
  fprintf(stderr, "ARRAY_COUNT(constellation_data) %lu\n", ARRAY_COUNT(constellation_data));
  for (u32 i=0; i<ARRAY_COUNT(constellation_data); i++) {
    const struct constellation_data_element *e = &constellation_data[i];
    fprintf(stderr, "  e->code_count %d - constellation_code_counts[%d] = %u\n",
        e->code_count, i, constellation_code_counts[i]);
...
START_TEST(test_signal_from_index)
{
  for (u32 i=0; i<ARRAY_COUNT(code_data); i++) {
    ...
    for (u16 code_index = 0; code_index < sat_count; code_index++) {
      ...
      fprintf(stderr, "  sid_from_code_index(%d, %d) = {%d, %d}\n",
          code, code_index, sid.sat, sid.code);
...
START_TEST(test_signal_properties)
{
  const struct test_case
  {
    ...
    char str[SID_STR_LEN_MAX];
      ...
      {
        .sid = {
          .code = CODE_GLO_L1OF, .sat = 29
        },
        .valid = false
      },
      {
        .sid = {
          .code = CODE_GLO_L2OF, .sat = 0
        },
        .valid = false,
      },
      {
        .sid = {
          .code = CODE_GLO_L2OF, .sat = 1
        },
        .valid = true,
        .str = "GLO L2OF 1"
      },
      ...
      {
        .sid = {
          .code = CODE_GLO_L2OF, .sat = 29
        },
        .valid = false
      },
      {
        .sid = {
          .code = CODE_GPS_L1P, .sat = 0
        },
        .valid = false,
        .str = "GPS L1P 0"
      },
  ...
  for (u32 i=0; i<sizeof(test_cases) / sizeof(test_cases[0]); i++) {
    ...
    if (valid) {
      ...
      fail_unless(sid_is_equal(t->sid, sid),
                  "test signal %d code index conversion failed", sid.code);
...
START_TEST(test_signal_compare)
{
  ...
  for (u32 i=0; i<ARRAY_COUNT(code_data); i++) {
    ...
    for (u16 sat_index = 0; sat_index < sat_count; sat_index++) {
      ...
      fprintf(stderr, "  sids[%d / %d] = sid_from_code_index(%d, %d) = {%d, %d}\n",
          signal_index, NUM_SIGNALS, code, sat_index, sid.sat, sid.code);
  ...
  for (u32 i=1; i<NUM_SIGNALS; i++) {
    fail_unless(!(sid_is_equal(sids[i], sids[i-1])&&!code_equiv(sids[i].code, sids[i-1].code)),
...
START_TEST(test_code_equivalence)
{
  ...
  for (u32 i = 0; i < sizeof(test_cases) / sizeof(test_cases[0]); i++) {
    ...
    fail_unless(code_equiv(t->sid_a.code, t->sid_b.code) == t->equiv,
                ...
                t->str, t->equiv, code_equiv(t->sid_a.code, t->sid_a.code));
  }
}
END_TEST
  ...
  for (u16 sat = GLO_FIRST_PRN; sat <= NUM_SATS_GLO; sat++) {
    for (u16 fcn = GLO_MIN_FCN; fcn <= GLO_MAX_FCN; fcn++) {
      ...
      fail_unless((GLO_L2_HZ + (glo_map_get_fcn(sid) - GLO_FCN_OFFSET)
                   * GLO_L2_DELTA_HZ) ==
                  carr_freq, "Glonass L2 carrier error");
      /* now L1 */
      ...
      fail_unless((GLO_L1_HZ + (glo_map_get_fcn(sid) - GLO_FCN_OFFSET)
                   * GLO_L1_DELTA_HZ) ==
                  carr_freq, "Glonass L1 carrier error");
    }
  }
}
END_TEST
...
START_TEST(test_signal_sid_to_lambda)
{
  ...
  for (u16 orb_slot = GLO_FIRST_PRN; orb_slot <= NUM_SATS_GLO; orb_slot++) {
    for (u16 fcn = GLO_MIN_FCN; fcn <= GLO_MAX_FCN; fcn++) {
      ...
      lambda = sid_to_lambda(sid);
      fail_unless((GPS_C / (GLO_L2_HZ + (glo_map_get_fcn(sid) - GLO_FCN_OFFSET)
                            * GLO_L2_DELTA_HZ)) == lambda, "Glonass L2 wavelength error");
      /* now L1 */
      mesid = construct_mesid(CODE_GLO_L1OF, fcn);
      sid = construct_sid(CODE_GLO_L1OF, orb_slot);
      lambda = sid_to_lambda(sid);
      fail_unless((GPS_C / (GLO_L1_HZ + (glo_map_get_fcn(sid) - GLO_FCN_OFFSET)
                            * GLO_L1_DELTA_HZ)) == lambda, "Glonass L1 wavelength error");
    }
  }
}
END_TEST
...
START_TEST(test_signal_code_to_chip_rate)
{
  ...
  fail_unless(GPS_CA_CHIPPING_RATE == chip_rate, "CODE_SBAS_L1CA chip rate error");

  chip_rate = code_to_chip_rate(CODE_GPS_L2CM);
  fail_unless(GPS_CA_CHIPPING_RATE == chip_rate, "CODE_GPS_L2CM chip rate error");
...
START_TEST(test_signal_code_requires_direct_acq)
{
  ...
  fail_unless(false == requires, "CODE_GPS_L2CM requires code_requires_direct_acq");
}
END_TEST
...
START_TEST(test_sid_system_check)
{
  for (u8 i = 0; i < CODE_COUNT; i++)
  {
    ...
    bool glo = (i == CODE_GLO_L1OF) || (i == CODE_GLO_L2OF) || (i == CODE_GLO_L1P) || (i == CODE_GLO_L2P);
    ...
    bool sbas = (i == CODE_SBAS_L1CA) || (i == CODE_AUX_SBAS) ||
        (i == CODE_SBAS_L5I) || (i == CODE_SBAS_L5Q) || (i == CODE_SBAS_L5X);
