...
START_TEST(test_pvt_failed_repair)
{
  ...
  calc_PVT(n_used, nms, &tor, false, true, ALL_CONSTELLATIONS, &soln, &dops,
           &raim_removed_sids);
...
START_TEST(test_pvt_raim_singular)
{
  ...
  s8 code = calc_PVT(n_used, nms, &tor, false, true, ALL_CONSTELLATIONS, &soln,
                     &dops, &raim_removed_sids);
  fail_unless(code == -4,
    "Return code should be -4 (RAIM failed). Saw: %d\n", code);
}
END_TEST
...
START_TEST(test_pvt_vel_repair)
{
  ...
  navigation_measurement_t nms[6] =
    {nm2, nm3, nm4, nm5, nm6b, nm7};

  s8 code = calc_PVT(n_used, nms, &tor, false, false, ALL_CONSTELLATIONS, &soln,
                     &dops, &raim_removed_sids);
  fail_unless(code == 1,
    "Return code should be 1 (pvt repair). Saw: %d\n", code);
  fail_unless(soln.n_sigs_used == n_used - 1,
...
START_TEST(test_pvt_repair_multifailure)
{
  ...
  navigation_measurement_t nms[8] =
  ...
  fail_unless(soln.n_sigs_used == n_used - 2,
    "n_sigs_used should be %u. Saw: %u\n", n_used - 2, soln.n_sigs_used);
  fail_unless(soln.n_sats_used == n_used - 2,
    "n_sats_used should be %u. Saw: %u\n", n_used - 2, soln.n_sats_used);
  fail_unless(sid_set_contains(&raim_removed_sids, expected_removed_sid),
    "Unexpected RAIM removed SID!\n");
}
END_TEST
...
START_TEST(test_pvt_raim_gps_l1ca_only)
{
  ...
  s8 code = calc_PVT(n_used, nms, &tor, false, false, GPS_L1CA_WHEN_POSSIBLE,
                     &soln, &dops, &raim_removed_sids);
  fail_unless(code == 1, "Return code should be 1 (pvt repair). Saw: %d\n",
              code);
  fail_unless(soln.n_sigs_used == n_gps_l1ca - 1,
    "n_sigs_used should be %u. Saw: %u\n", n_gps_l1ca - 1, soln.n_sigs_used);
  fail_unless(soln.n_sats_used == n_gps_l1ca - 1,
    "n_sats_used should be %u. Saw: %u\n", n_gps_l1ca - 1, soln.n_sats_used);
  fail_unless(sid_set_contains(&raim_removed_sids, expected_removed_sid),
    "Unexpected RAIM removed SID!\n");
}
END_TEST
...
START_TEST(test_pvt_outlier_gps_l1ca_only)
{
  ...
  fail_unless(soln.n_sigs_used == n_gps_l1ca,
    "n_sigs_used should be %u. Saw: %u\n", n_gps_l1ca, soln.n_sigs_used);
  fail_unless(soln.n_sats_used == n_gps_l1ca,
    "n_sats_used should be %u. Saw: %u\n", n_gps_l1ca, soln.n_sats_used);
  fail_unless(sid_set_contains(&raim_removed_sids, expected_removed_sid),
    "Unexpected RAIM removed SID!\n");
}
END_TEST
...
START_TEST(test_pvt_flag_outlier_bias) {
  ...
  navigation_measurement_t nms[9] = {nm2, nm3, nm4, nm5, nm6,
                                     nm7, nm8, nm10_bias, nm11_bias};
  ...
  code = calc_PVT(n_used, nms, &tor, false, false, GPS_L1CA_WHEN_POSSIBLE,
                  &soln, &dops, &raim_removed_sids);

  gnss_signal_t expected_removed_sid = {.code = CODE_GPS_L2CM, .sat = 8};

  fail_unless(code == 1, "Return code should be 1 (RAIM repair). Saw: %d\n",
              code);
  fail_unless(soln.n_sigs_used == n_gps_l1ca,
              "n_sigs_used should be %u. Saw: %u\n", n_gps_l1ca,
              soln.n_sigs_used);
...
START_TEST(test_disable_pvt_raim)
{
  ...
  /* disable raim check */
  s8 code = calc_PVT(n_used, nms, &tor, true, true, ALL_CONSTELLATIONS, &soln,
                     &dops, &raim_removed_sids);
  fail_unless(code == 2,
...
START_TEST(test_disable_pvt_velocity)
{
  ...
  fail_unless(soln.valid == 1,
    "Solution should be valid!");
  fail_unless((soln.vel_ned[0] == 0.0) &&
                (soln.vel_ned[1] == 0.0) && (soln.vel_ned[2] == 0.0),
              "Velocity NED was not zero.  "
              "Saw: %.5f, %.5f, %.5f\n",
              soln.vel_ned[0], soln.vel_ned[1], soln.vel_ned[2]);
  fail_unless((soln.vel_ecef[0] == 0.0) &&
                (soln.vel_ecef[1] == 0.0) && (soln.vel_ecef[2] == 0.0),
              "Velocity ECEF was not zero.  "
              "Saw: %.5f, %.5f, %.5f\n",
              soln.vel_ecef[0], soln.vel_ecef[1], soln.vel_ecef[2]);
}
END_TEST
...
START_TEST(test_count_sats)
{
  ...
  /* disable raim check */
  s8 code = calc_PVT(n_used, nms, &tor, true, false, ALL_CONSTELLATIONS, &soln,
                     &dops, &raim_removed_sids);
  fail_unless(code >= 0,
    "Return code should be >=0 (success). Saw: %d\n", code);
  fail_unless(soln.valid == 1,
...
START_TEST(test_count_sats_l1ca_only)
{
  ...
  /* disable raim check */
  s8 code = calc_PVT(n_used, nms, &tor, true, false, GPS_L1CA_WHEN_POSSIBLE,
                     &soln, &dops, &raim_removed_sids);
  fail_unless(code >= 0, "Return code should be >=0 (success). Saw: %d\n",
              code);
  fail_unless(soln.valid == 1,
